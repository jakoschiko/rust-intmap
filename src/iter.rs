...
pub struct Iter<'a, K: 'a, V: 'a> {
    inner: IterFlatten<SliceIter<'a, SmallVec<[(K, V); 2]>>>,
}

impl<'a, K, V> Iter<'a, K, V> {
    pub(crate) fn new(vec: &'a [SmallVec<[(K, V); 2]>]) -> Self {
        Iter {
...
pub struct IterMut<'a, K: 'a, V: 'a> {
    inner: IterFlatten<SliceIterMut<'a, SmallVec<[(K, V); 2]>>>,
}

impl<'a, K, V> IterMut<'a, K, V> {
    pub(crate) fn new(vec: &'a mut [SmallVec<[(K, V); 2]>]) -> IterMut<'a, K, V> {
        IterMut {
...
pub struct IntoIter<K, V> {
    inner: IterFlatten<VecIntoIter<SmallVec<[(K, V); 2]>>>,
}

impl<K, V> IntoIter<K, V> {
    pub(crate) fn new(vec: Vec<SmallVec<[(K, V); 2]>>) -> Self {
        IntoIter {
...
pub struct Drain<'a, K: 'a, V: 'a> {
    count: &'a mut usize,
    inner: IterFlatMap<
        SliceIterMut<'a, SmallVec<[(K, V); 2]>>,
        SmallVecDrain<'a, [(K, V); 2]>,
        fn(&mut SmallVec<[(K, V); 2]>) -> SmallVecDrain<[(K, V); 2]>,
    >,
}

impl<'a, K, V> Drain<'a, K, V> {
    pub(crate) fn new(
        vec: &'a mut [SmallVec<[(K, V); 2]>],
        count: &'a mut usize,
    ) -> Drain<'a, K, V> {
        Drain {
            count,
