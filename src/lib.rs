...
/// A hashmap that maps `K` to `V`.
#[derive(Clone)]
pub struct IntMap<K: IntKey, V> {
...
impl<K: IntKey, V> IntMap<K, V> {
    ...
    pub fn insert(&mut self, key: K, value: V) -> Option<V> {
        ...
        let int = key.to_int();
        let ix = int.calc_index(self.mod_mask);

        let vals = &mut self.cache[ix];
        let pos = vals.iter().position(|kv| kv.0 == int);
    ...
    pub fn get(&self, key: K) -> Option<&V> {
        ...
        vals.iter().find_map(|kv| (kv.0 == int).then(|| &kv.1))
    }
    ...
    pub fn get_mut(&mut self, key: K) -> Option<&mut V> {
        ...
        return vals
            .iter_mut()
            .find_map(|kv| (kv.0 == int).then(move || &mut kv.1));
    }
    ...
    fn increase_cache(&mut self) {
        ...
        let mut vec: Vec<Vec<(K::Int, V)>> = (0..new_lim).map(|_| Vec::new()).collect();
        std::mem::swap(&mut self.cache, &mut vec);

        for k in vec.into_iter().flatten() {
            let ix = k.0.calc_index(self.mod_mask);
...
impl<K: IntKey, V> PartialEq for IntMap<K, V>
where
    V: PartialEq,
{
...
impl<K: IntKey, V> Eq for IntMap<K, V> where V: Eq {}

// ***************** Debug *********************

impl<K: IntKey, V> std::fmt::Debug for IntMap<K, V>
where
    K: std::fmt::Debug,
    V: std::fmt::Debug,
{
